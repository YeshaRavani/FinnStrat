...
export function StrategyCard({ item, rank, selected, compared, onSelect, onCompare }: Props) {
  const actualLoanAmount = item.normal_simulation.loan_created > 0 ? item.normal_simulation.loan_created : item.strategy.loan_amount;
  const loan = loanSummary(item.strategy, item.maturity_month, actualLoanAmount);
  ...
  return (
    <article className={`strategy-card ${rank === 1 ? "featured" : ""} ${selected ? "selected" : ""}`}>
      ...
      <div className="card-stats">
        <div><span>GOAL PURCHASE</span><strong>{item.maturity_month ? `Month ${item.maturity_month}` : "Not reached"}</strong></div>
        <div><span>LIQUIDITY</span><strong>{Math.round(item.liquidity_score)}<small>/100</small></strong></div>
        <div><span>LOAN</span><strong>₹{Math.round(actualLoanAmount).toLocaleString("en-IN")}</strong></div>
        ...
        <div><span>LOAN TERMS</span><strong>{actualLoanAmount > 0 ? `${item.strategy.loan_term_months} mo / ${(item.strategy.annual_interest_rate * 100).toFixed(1)}%` : "No loan"}</strong></div>
        ...
        <div><span>LOAN PAYOFF</span><strong>{loan.payoffMonth ? `Month ${loan.payoffMonth}` : "No loan"}</strong></div>
