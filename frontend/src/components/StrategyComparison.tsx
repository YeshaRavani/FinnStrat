    ...
    <div className="comparison-wrap">
      <table className="comparison-table">
        ...
        <tbody>
          ...
          <tr><th scope="row">Goal purchase</th>{strategies.map(item => <td key={item.strategy.id}>{item.maturity_month ? `Month ${item.maturity_month}` : "Not reached"}</td>)}</tr>
          <tr><th scope="row">Liquidity</th>{strategies.map(item => <td key={item.strategy.id}>{Math.round(item.liquidity_score)}/100</td>)}</tr>
          <tr><th scope="row">Loan</th>{strategies.map(item => { const actualLoanAmount = item.normal_simulation.loan_created > 0 ? item.normal_simulation.loan_created : item.strategy.loan_amount; return <td key={item.strategy.id}>₹{Math.round(actualLoanAmount).toLocaleString("en-IN")}</td>; })}</tr>
          ...
          <tr><th scope="row">Loan terms</th>{strategies.map(item => { const actualLoanAmount = item.normal_simulation.loan_created > 0 ? item.normal_simulation.loan_created : item.strategy.loan_amount; return <td key={item.strategy.id}>{actualLoanAmount > 0 ? `${item.strategy.loan_term_months} mo / ${(item.strategy.annual_interest_rate * 100).toFixed(1)}%` : "No loan"}</td>; })}</tr>
          <tr><th scope="row">Estimated EMI</th>{strategies.map(item => { const actualLoanAmount = item.normal_simulation.loan_created > 0 ? item.normal_simulation.loan_created : item.strategy.loan_amount; const loan = loanSummary(item.strategy, item.maturity_month, actualLoanAmount); return <td key={item.strategy.id}>{loan.emi ? `₹${Math.round(loan.emi).toLocaleString("en-IN")}` : "No loan"}</td>; })}</tr>
          <tr><th scope="row">Estimated interest</th>{strategies.map(item => { const actualLoanAmount = item.normal_simulation.loan_created > 0 ? item.normal_simulation.loan_created : item.strategy.loan_amount; const loan = loanSummary(item.strategy, item.maturity_month, actualLoanAmount); return <td key={item.strategy.id}>{loan.totalInterest ? `₹${Math.round(loan.totalInterest).toLocaleString("en-IN")}` : "No loan"}</td>; })}</tr>
          <tr><th scope="row">Loan payoff</th>{strategies.map(item => { const loan = loanSummary(item.strategy, item.maturity_month, item.normal_simulation.loan_created > 0 ? item.normal_simulation.loan_created : item.strategy.loan_amount); return <td key={item.strategy.id}>{loan.payoffMonth ? `Month ${loan.payoffMonth}` : "No loan"}</td>; })}</tr>
